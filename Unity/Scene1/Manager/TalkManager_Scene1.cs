...
// 대화 매니저
public class TalkManager_Scene1 : MonoBehaviour {
    // 게임 매니저
    public GameManager_Components GameManager;
    // 대화 내용 데이터
    public TalkDictionary_Scene1 TalkDictionary;
    // 퀘스트 매니저
    ...
    RectTransform rectMoney;
    RectTransform rectMap;
    RectTransform rectJoystcik;
    RectTransform rectSetting;


    bool isMoneyBounce = false;
    bool isMapBounce = false;
    bool isJoystickBounce = false;
    bool isSettingBounce = false;
    ...
    void Start()
    {
        rectMoney = money.GetComponent<RectTransform>();
        rectMap = map.GetComponent<RectTransform>();
        rectJoystcik = joystick.GetComponent<RectTransform>();
        rectSetting = setting.GetComponent<RectTransform>();
    }
    ...
    void Update()
    {
        ...
        if (isJoystickBounce)
        {
            rectMap.localScale = new Vector3(1, 1, 1);
            Bounce(rectJoystcik);
        }
        if (isSettingBounce)
        {
            rectJoystcik.localScale = new Vector3(1, 1, 1);
            Bounce(rectSetting);
        }
    ...
    void Talk(int id, bool isNpc) {
        ...
        if (talkDataArray[0].Contains("#####"))
            talkDataArray[0] = talkDataArray[0].Replace("#####", playerNameManager.GetPlayerName());

        if (talkDataArray[1].Contains("#####"))
            talkDataArray[1] = talkDataArray[1].Replace("#####", playerNameManager.GetPlayerName());
        ...
        if (questTalkIndex == 10 && talkIndex == 3)
        {
            popupManager.Appear();
            isNameSetting = true;
        }

        if (questTalkIndex == 10 && talkIndex == 7)
        {
            startBox.SetActive(false);
        }
        // Money 버튼 바운스
        if(questTalkIndex == 21 && talkIndex == 1)
        {
            isMoneyBounce = true;
        }
        // Money 버튼 바운스 해제 및 크기 원래대로
        if(questTalkIndex == 21 && talkIndex == 2)
        {
            isMoneyBounce = false;
            rectMoney.localScale = new Vector3(1, 1, 1);
        }
        // Map 버튼 바운스
        if (questTalkIndex == 21 && talkIndex == 3)
        {
            isMapBounce = true;
        }
        // Map 버튼 바운스 해제 및 크기 원래대로
        // 사용법 버튼 바운스
        if (questTalkIndex == 21 && talkIndex == 4)
        {
            isMapBounce = false;
            isJoystickBounce = true;
        }
        // 사용법 버튼 바운스 해제 및 크기 원래대로
        // 설정 버튼 바운스
        if (questTalkIndex == 21 && talkIndex == 5)
        {
            isJoystickBounce = false;
            isSettingBounce = true;
        }
        // 설정 버튼 바운스 해제 및 크기 원래대로
        if(questTalkIndex == 21 && talkIndex == 6 )
        {
            isSettingBounce = false;
            rectSetting.localScale = new Vector3(1, 1, 1);
        }
