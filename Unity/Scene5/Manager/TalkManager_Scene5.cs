...
// 대화 매니저
public class TalkManager_Scene5 : MonoBehaviour {
    public GameObject letter;
    // 게임 매니저
    public GameManager_Components GameManager;
    // 대화 내용 데이터
    public TalkDictionary_Scene5 TalkDictionary;
    // 퀘스트 매니저
    ...
    IEnumerator Typing(string text, int questTalkIndex, int id) {
        ...
        if (!(questTalkIndex + id == 2 + 1000))
        {
            if (QuestManager.questId == 20 && QuestManager.questActionIndex == 1 && talkIndex == 1)
            {
                Invoke("nextChat1", 20.0f);
            }
            else
            {
                nextChat1();
